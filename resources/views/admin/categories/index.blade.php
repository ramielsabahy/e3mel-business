...
@section('create-path', route('categories.create'))
...
{{--                            <th>Logo</th>--}}
                        ...
                        @foreach($categories as $category)
                            <tr>
                                <td>
                                    <p class="list-item-heading">{{$category->name}}</p>
                                </td>
                                <td>
                                    <div class="custom-switch custom-switch-primary mb-2">
                                        <input onchange="toggleStatus('{{$category->id}}','{{$category->active}}')" class="custom-switch-input customers-switch" id="switch{{$category->id}}" type="checkbox" data-status="{{$category->active}}" data-id="{{$category->id}}" @if($category->is_active) checked @endif>
                                        <label class="custom-switch-btn" for="switch{{$category->id}}"></label>
                                    </div>
                                </td>
                                <td>
...
@section('js')
    ...
    <script>
        ...
        function toggleStatus(id, status){
            Swal.fire({
                title: "Are you sure you want to toggle this category status ?",
                ...
                if (result.isConfirmed){
                    $.ajax({
                        url: "{{url('/admin/toggle-category/')}}"+"/"+id,
                        method: "GET"
                    }).done(function(){
                        Swal.fire(
                            'Done!',
